...
def run_command(
    ...
    """Run a shell command locally and return stdout, stderr, and return code.

    Used by scanning sub-agents to execute gosec, semgrep, golangci-lint,
    gitleaks, go test, etc. directly on the host machine where the repo lives.

    Rules:
    - cwd must be an absolute path inside the repo (e.g. /abs/path/to/repo/api)
    - Read-only analysis commands only — do NOT write to the repo
    ...
    if cwd and cwd.strip():
        ...
        if not cwd_path.is_dir():
            return {"success": False, "error": f"cwd is not a directory: {cwd}"}
    ...
    try:
        ...
        return {
            ...
            "truncated": len(result.stdout) > 50000 if result.stdout else False,
        }
    except subprocess.TimeoutExpired:
        return {
            "success": False,
